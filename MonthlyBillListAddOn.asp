...
<form method="post" name="frmSearch" Action="MonthlyBillListAddOn.asp">
<CENTER>
<%
...
<table cellspadding="1" cellspacing="0" width="500px" border="1" align="center">
<tr align="Center">
...
<table border="1" bordercolor="#000000" cellpadding="2" cellspacing="0" width="100%"  class="FontText">
    <TR BGCOLOR="#330099" align="center">
         <TD rowspan="2" width="15px" align="right"><strong><label STYLE=color:#FFFFFF>No.</label></strong></TD>
...
-->
         <TD rowspan="2"><strong><label STYLE=color:#FFFFFF>Pay.gov form</label></strong></TD>
    </TR>
    <tr BGCOLOR="#330099" align="center">
...
-->
<%
		if (DataRS("ProgressID") = 6) or (DataRS("ProgressID") = 7) then
			Amount_ = 0
		else
			Amount_ = DataRS("TotalBillingAmountPrsDlr")
		end if
%>
		<td>
			<a href="ePaymentForm.asp?Amount=<%=Amount_ %>" target="_blank">View</a>
		</td>
	   </TR>
...
%>		
			...
			<%Else%>
				<a href="MonthlyBillListAddOn.asp?PageIndex=<%=PageNo%>&sMonthP=<%=sMonthP%>&sYearP=<%=sYearP%>&eMonthP=<%=eMonthP%>&eYearP=<%=eYearP%>"><%=PageNo%></a>&nbsp;
<%	
			End If						
			PageNo=PageNo+1
		Loop

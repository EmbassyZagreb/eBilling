...
If (UserRole_ = "Admin") or (UserRole_ = "Voucher") Then
	...
	strsql = "spGetPaymentList '2','" & MonthP & "','" & YearP & "','','" & EmpName_ & "','" & OfficeSection_ & "'," & Outstanding_ & ",'" & Status_ & "'"
...
%>
   	<TR bgcolor="<%=bg%>">
		...
		<%If (DataRS("ProgressID")<=4) and (trim(DataRS("Status")) <>"Paid") Then %>			
			<A HREF="HomePhoneSelfPaymentApproval.asp?HomePhone=<%=DataRS("PhoneNumber")%>&PageIndex=<%=PageIndex%>&MonthP=<%=MonthP%>&YearP=<%=YearP%>" >Update</A>
		<%Else%>
			&nbsp;<%=DataRS("Status")%>
		<%End If%>
		</td> 
...
%>		
			...
			<%Else%>
				<a href="HomePhoneSelfPaymentList.asp?PageIndex=<%=PageNo%>&EmpName=<%=EmpName_%>&OfficeSection=<%=OfficeSection_%>&Outstanding=<%=Outstanding_%>&Status=<%=Status_%>&MonthP=<%=MonthP%>&YearP=<%=YearP%>"><%=PageNo%></a>&nbsp;
<%	
			End If						
			PageNo=PageNo+1
		Loop

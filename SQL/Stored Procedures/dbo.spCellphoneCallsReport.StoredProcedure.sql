/****** Object:  StoredProcedure [dbo].[spCellphoneCallsReport]    Script Date: 12/02/2014 15:00:12 ******/
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER OFF
GO
CREATE Procedure [dbo].[spCellphoneCallsReport] 
	@Type varchar(1)=Null
...
If @Type='1'
Begin
	Set @Command='Select * From vwCellPhoneCall 
...
If @Type='2'
Begin
	Set @Command='Select isNull(sum(CallDurationSecond),0) As TotalDuration, isNull(sum(Cost),0) As TotalCost from vwCellPhoneCall 
		Where DialedDateTime>='''+@StartDate+''' And DialedDateTime<='''+@EndDate+''' 
		And (Post = '''+ @Post + ''' or '''+@Post+''' =''A'')  
		And (Agency = '''+ @Agency + ''' or '''+@Agency+''' =''A'') 
		And (Office = '''+ @Office + ''' or '''+@Office+''' =''A'') 

...
<!--#include file="Header.inc" -->
  <TR>
  	<TD COLSPAN="4" ALIGN="center" Class="title">A/R REMINDER</TD>
   </TR>
<tr>
...
strsql = "Select * From vwARReminderRpt Where YearP+MonthP>='" & sPeriod & "' and YearP+MonthP<='" & ePeriod & "'"
strFilter=""
If Agency_ <> "X" then
...
<table width="100%">
<tr>
	<td align="Left"><input type="button" name="btnExport" value="Export to Excel" onClick="javascript:document.location.href('ARReminderPrint.asp?sMonthP=<%=sMonthP%>&sYearP=<%=sYearP%>&eMonthP=<%=eMonthP%>&eYearP=<%=eYearP%>&Agency=<%=Agency_%>&Section=<%=Section_%>&EmpID=<%=EmpID_%>&Status=<%=Status_%>&SortBy=<%=SortBy_%>&Order=<%=Order_%>');"/></td>
...
<table border="1" bordercolor="#000000" cellpadding="2" cellspacing="0" width="100%"  class="FontText">
    <TR BGCOLOR="#330099" align="center">
         <TD rowspan="2" width="3px" align="right"><strong><label STYLE=color:#FFFFFF>No.</label></strong></TD>
         <TD rowspan="2" width="15%"><strong><label STYLE=color:#FFFFFF>Employee Name</label></strong></TD>
         <TD rowspan="2"><strong><label STYLE=color:#FFFFFF>Billing Period</label></strong></TD>
	 ...
	 <TD colspan="5"><strong><label STYLE=color:#FFFFFF>Billing Amount (Kn.)</label></strong></TD>
         <TD rowspan="2"><strong><label STYLE=color:#FFFFFF>A/R Aging</label></strong></TD>
         <TD rowspan="2"><strong><label STYLE=color:#FFFFFF>Email</label></strong></TD>
         <TD rowspan="2"><strong><label STYLE=color:#FFFFFF>Send Status</label></strong></TD>
...
%>		
			...
			<%Else%>
				<a href="ARReminder.asp?PageIndex=<%=PageNo%>&sMonthP=<%=sMonthP%>&sYearP=<%=sYearP%>&eMonthP=<%=eMonthP%>&eYearP=<%=eYearP%>&Agency=<%=Agency_%>&Section=<%=Section_%>&EmpID=<%=EmpID_%>&Status=<%=Status_%>&SortBy=<%=SortBy_%>&Order=<%=Order_%>"><%=PageNo%></a>&nbsp;
<%	
			End If						
			PageNo=PageNo+1
		Loop

...
function ClearFilter()
{
	...
	document.forms['frmSearch'].elements['SortList'].value ='Location';
}
</script>
...
<CENTER>
  ...
  <TR>
  	<TD COLSPAN="4" ALIGN="center" Class="title">NO LINE USAGES REPORT</TD>
   </TR>
<tr>
			...
			<tr>
				...
				<td>
					<Select name="SortList">
						<Option value="PhoneNumber" <%if SortBy_ ="PhoneNumber" then %>Selected<%End If%> >Extension</Option>
						<Option value="EmpName" <%if SortBy_ ="EmpName" then %>Selected<%End If%> >Employee Name/Lcoation</Option>
...
<!--					<input type="Button" name="btnBack" value="Back" onClick="Javascript:document.location.href('BillingReportList.asp');"> -->
	...
	<form method="post" name="frmLineUsagesList" action="NoLineUsagesPrint.asp?Post=<%=Post_%>&StartDate=<%=StartDate_ %>&EndDate=<%=EndDate_%>&SortBy=<%=SortBy_%>&Order=<%=Order_%>">
	...
	<TR BGCOLOR="#000099" align="center" cellpadding="0" cellspacing="0" >
		...
		<TD><strong><label STYLE=color:#FFFFFF>Employee Name</label></strong></TD>
	       	<TD width="14%"><strong><label STYLE=color:#FFFFFF>Post</label></strong></TD>
	</TR>    
	<% 
		dim no_  
	'	no_ = 1 
	...
	%>		
				...
				<%Else%>
					<a href="NoLineUsagesReport.asp?PageIndex=<%=PageNo%>&Post=<%=Post_%>&StartDate=<%=StartDate_ %>&EndDate=<%=EndDate_%>&SortBy=<%=SortBy_%>&Order=<%=Order_%>"><%=PageNo%></a>&nbsp;
	<%	
				End If						
				PageNo=PageNo+1
			Loop

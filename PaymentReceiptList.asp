...
If (UserRole_ = "Admin") or (UserRole_ = "Voucher") or (UserRole_ = "FMC") or (UserRole_ = "Cashier") Then
	...
	strsql = "spGetPaymentReceipt '" & sPeriod & "','" & ePeriod & "','%" & EmpName_ & "%'," & Outstanding_ & ",'" & OfficeSection_  & "'," & Status_ 
...
<table border="1" bordercolor="#EEEEEE" cellpadding="2" cellspacing="0" width="100%"  class="FontText">
    <TR BGCOLOR="#330099" align="center" height=26>
	 ...
	 <TD ><strong><label STYLE=color:#FFFFFF>Billing Amount (Kn.)</label></strong></TD>
         <TD ><strong><label STYLE=color:#FFFFFF>Paid Amount</label></strong></TD>
         <TD ><strong><label STYLE=color:#FFFFFF>Paid Date</label></strong></TD>
         <TD >
...
<%	End If%>
		...
		<TD>&nbsp;<%= Left(DataRS("PaidDate"),11) %></font></TD>
...
%>		
			...
			<%Else%>
				...
				<A HREF="PaymentReceiptList.asp?PageIndex=<%=PageNo%>&Outstanding=<%=Outstanding_%>
					&sMonthP=<%=sMonthP%>&sYearP=<%=sYearP%>&eMonthP=<%=eMonthP%>&eYearP=<%=eYearP%>&EmpName=<%=EmpName_%>&Status=<%=Status_ %>
					&OfficeSection=<%=OfficeSection_%>"><%=PageNo%></A>&nbsp;

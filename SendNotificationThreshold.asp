...
function ClearFilter()
{
	...
	document.forms['frmSearch'].elements['cmbBillType'].value="X";
...
<%					StatusRS.MoveNext
		...
		<tr>
			...
			<td>:</td>
			<td>
				<input type="input" name="txtEmailAddress" value="<%=EmailAddress_%>" size=30 />
			</td>	
...
<%		End If%>
	        ...
	        <TD><a title="click to update the status" href="ChangeProgressStatus.asp?EmpID=<%=DataRS("EmpID")%>&MonthP=<%= DataRS("MonthP")%>&YearP=<%= DataRS("YearP")%>" target="_blank"><FONT color=#330099 size=2><%=DataRS("ProgressDesc") %> </font></a></TD>
	    </TR>

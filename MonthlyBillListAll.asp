...
%>
<form method="post" name="frmSearch" Action="MonthlyBillListAll.asp">
...
%>
<form method="post" name="frmData" Action="MonthlyBillListAllPrint.asp" onsubmit="return ValidateForm()">
<table width="100%">
<tr>
<!--	<td align="left"><input type="button" name="btnExport" value="Export to Excel" onClick="javascript:document.location.href('MonthlyBillListAllPrint.asp?sMonthP=<%=sMonthP%>&sYearP=<%=sYearP%>&eMonthP=<%=eMonthP%>&eYearP=<%=eYearP%>&EmpID=<%=DataRS("EmpID")%>');"/></td> -->
...
%>		
			...
			<%Else%>
				<a href="ARPaymentReport.asp?PageIndex=<%=PageNo%>&sMonthP=<%=sMonthP%>&sYearP=<%=sYearP%>&eMonthP=<%=eMonthP%>&eYearP=<%=eYearP%>&Agency=<%=Agency_%>&Section=<%=Section_%>&EmpID=<%=EmpID_%>&Status=<%=Status%>"><%=PageNo%></a>&nbsp;
<%	
			End If						
			PageNo=PageNo+1
		Loop

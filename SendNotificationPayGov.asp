...
<!--#include file="Header.inc" -->
  <TR>
  	<TD COLSPAN="4" ALIGN="center" Class="title">BILLING NOTIFICATION - PayGov.com</TD>
   </TR>
<tr>
...
%>
<form method="post" name="frmSearch" Action="SendNotificationPayGov.asp">
...
<tr align="Center">
	<td colspan="2" align="center">
		...
		<tr>
			...
			<td>:</td>
			<td colspan="4">
				<input type="radio" name="rbPeriod" value="X" <%if rbPeriod_ = "X" then%>checked<%end if%> >All</input>
<!--				<input type="radio" name="rbPeriod" value="S" <%if rbPeriod_ = "S" then%>checked<%end if%> >Custom :</input>
...
<%					EmpRS.MoveNext
			...
			<td>
<%
 				strsql ="select ProgressID, ProgressDesc from ProgressStatus Where ProgressID in(4,8) Order By OrderNo"
...
if rbPeriod_ = "X" then
	'strsql = "Select * From vwMonthlyBilling "
	strFilter=""
else
	'strsql = "Select * From vwMonthlyBilling"
	strFilter=" and YearP+MonthP>='" & sPeriod & "' and YearP+MonthP<='" & ePeriod & "'"
end if
...
%>
<form method="post" name="frmARReminder" Action="SendNotificationPayGovSendMail.asp" onSubmit="return ValidateForm()">
<table width="100%">
<tr>
	<td align="Left"><input type="button" name="btnExport" value="Export to Excel" onClick="javascript:document.location.href('SendNotificationPayGovPrint.asp?Agency=<%=Agency_%>&Section=<%=Section_%>&SectionGroup=<%=SectionGroup_%>&SortBy=<%=SortBy_%>&Order=<%=Order_%>&EmpID=<%=EmpID_%>&BillType=<%=BillType%>&Status=<%=Status_%>&SentStatus=<%=SentStatus_%>');"/></td>
	<td align="Right"><input type="submit" name="btnSendNotice" value="Send Notice(s)" /></td>
</tr>
</table>
<%If BillType = "X" then %>
	<table border="1" bordercolor="#000000" cellpadding="2" cellspacing="0" width="110%"  class="FontText">
...
%>
	   ...
	   <TR bgcolor="<%=bg%>">
	        ...
	        <TD>&nbsp;<a href="BillingPayGovPreview.asp?EmpId=<%=DataRS("EmpId")%>&BillType=<%=BillType%>&sMonthP=<%=sMonthP%>&sYearP=<%=sYearP%>&eMonthP=<%=eMonthP%>&eYearP=<%=eYearP%>&rbPeriod=<%=rbPeriod_%>&ProgressID=<%=Status_%>" target="blank"><%=DataRS("EmpName") %></a></TD>
...
<%		If len(DataRS("EmailAddress"))>5 then %>
			<Input type="Checkbox" name="cbApproval" Value='<%=DataRS("EmpID")%>'>
<%		Else%>
			&nbsp;
<%		End If%>
		</td>

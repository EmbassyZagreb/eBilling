...
function validate_form()
{
	...
	if (document.frmSearch.txtOutstanding.value != "" )
	{
		...
		{
			msg = msg + "Invalid data type for Outstanding !!!\n"
			valid = false;
		}
	}
...
<!--#include file="Header.inc" -->
   <TR>
  	<TD COLSPAN="4" ALIGN="center" Class="title">OFFICE PHONE PAYMENT LIST</TD>
   </TR>
<tr>
...
If (UserRole_ <> "") Then
	...
	strsql = "spGetPaymentList '1','" & MonthP_ & "','" & YearP_ & "','','"   & LastName_ & "','" & FirstName_ & "','" & OfficeSection_ & "'," & Outstanding_ & ",'" & Status_ & "'"
...
<%
		...
		<td>
			<select name="cmbStatus">
				...
				<option value="S" <%if Status_ ="S" then%>Selected<%End If%>>Submitted</option>
				<option value="A" <%if Status_ ="A" then%>Selected<%End If%>>Approved</option>
				<option value="C" <%if Status_ ="C" then%>Selected<%End If%>>Correction</option>
...
<TR BGCOLOR="#000099" align="center" cellpadding="0" cellspacing="0" >
	...
	<TD width="15%"><strong><label STYLE=color:#FFFFFF>Agency / Office</label></strong></TD>
       	<TD width="8%"><strong><label STYLE=color:#FFFFFF>Phone Ext.</label></strong></TD>
	<TD width="12%"><strong><label STYLE=color:#FFFFFF>Personal Cost (kn)</label></strong></TD>
...
%>
   	<TR bgcolor="<%=bg%>">
		<td align="right"><%=No_%>&nbsp;</td>
        	...
        	<td align="right"><FONT color=#330099 size=2><%=formatnumber(DataRS("PersonalCost"),-1)%>&nbsp;</font></td> 
        	...
        	<td>&nbsp;
		<%If (DataRS("Status")="Approved") Then %>			
			<A HREF="OfficePhonePaymentApproval.asp?Extension=<%=DataRS("Extension")%>&PageIndex=<%=PageIndex%>&MonthP=<%=MonthP_%>&YearP=<%=YearP_%>">Payment</A>
		<%Else%>
			&nbsp;<%=DataRS("Status")%>
		<%End If%>
		</td> 
...
%>		
			...
			<%Else%>
				<a href=OfficePhonePaymentList.asp?PageIndex=<%=PageNo%>&LastName=<%=LastName_%>&FirstName=<%=FirstName_%>&OfficeSection=<%=OfficeSection_%>&Outstanding=<%=Outstanding_%>&Status=<%=Status_%>&MonthP=<%=MonthP_%>&YearP=<%=YearP_%>"><%=PageNo%></a>&nbsp;
<%	
			End If						
			PageNo=PageNo+1
		Loop

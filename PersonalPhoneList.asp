...
<!--#include file="Header.inc" -->
  <TR>
  	<TD COLSPAN="4" ALIGN="center" Class="title">Personal Phone List</TD>
   </TR>
<tr>
...
%>
	...
	<tr>
		<td>Name :</td>
		<td><input name="txtPhoneNumber" type="Input" size="50" Value='<%=PhoneNumber_%>'></td>
		<td>
...
<table width="90%">
<tr>
	<td><a href="PersonalPhoneEdit.asp?State=I">Add New Personal Phone</a></td>
</tr>
</table>
...
<TR BGCOLOR="#000099" align="center" cellpadding="0" cellspacing="0" >
	...
	<TD width="150px"><strong><label STYLE=color:#FFFFFF>Phone Number</label></strong></TD>
...
%>
   	<TR bgcolor="<%=bg%>">
		<td align="right"><%=No_%>&nbsp;</td>
	        <td>&nbsp;<FONT color=#330099 size=2><A HREF="PersonalPhoneEdit.asp?ID=<%=DataRS("ID")%>&State=E"><%= DataRS("PhoneNumber") %></A></font></td>
        	<td><FONT color=#330099 size=2>&nbsp;<%=DataRS("Remark")%></font></td> 
		</td> 
		<TD><a href="PersonalPhoneDeleteConfirm.asp?ID=<%=DataRS("ID")%>&State=E">Delete</a></TD>
  	 </TR>
<%   
		Count=Count +1
 		DataRS.movenext
...
%>		
			...
			<%Else%>
				<a href="PersonalPhoneList.asp?PageIndex=<%=PageNo%>&PhoneNumber=<%=PhoneNumber_%>"><%=PageNo%></a>&nbsp;
<%	
			End If						
			PageNo=PageNo+1
		Loop

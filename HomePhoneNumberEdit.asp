...
function validate_form()
{
	...
	if (document.frmHomePhone.txtPhoneNumber.value == "" )
	{
		msg = msg + "Please fill in Phone Number!!!\n"
		valid = false;
	}

	if (document.frmHomePhone.txtAlternateEmail.value != "" )
	{
		var alnum="a-zA-Z0-9";
		exp="^[^@\\s]+@(["+alnum+"+\\-]+\\.)+["+alnum+"]["+alnum+"]["+alnum+"]?$";
		emailregexp = new RegExp(exp);

		result = document.frmHomePhone.txtAlternateEmail.value.match(emailregexp);
		if (result == null)
		{
...
<!--#include file="Header.inc" -->
  <TR>
  	<TD COLSPAN="4" ALIGN="center" Class="title">HOME PHONE LIST</TD>
   </TR>
<tr>
...
<form method="post" name="frmHomePhone" action="HomePhoneNumberSave.asp"  onsubmit="return validate_form();"> 
<%  
 dim rst 
 dim strsql
 dim rst1
...
'response.write strsql 
  ...
  if not rst.eof then 
	...
	if State_ = "E" then
	        strsql = " select * from vwHomePhoneNumberList where ID = " & ID_ 
		...
		'response.write strsql 
		   ...
		   EmpID_ = rst1("EmpID")
		   CustNo_ = rst1("CustNo")
		   CustName_ = rst1("CustName")
		   Address_ = rst1("Address")
		   City_ = rst1("City")
		   EmailAddress_ = rst1("EmailAddress")
		   AlternateEmail_ = rst1("AlternateEmail")
		   Remark_ = rst1("Remark")
		   NoticeFlag_ = rst1("NoticeFlag")
	...
	<tr>
	  <td>Phone Number</td>
	  <td width="1%">:</td>
	  <td><input type="input" name="txtPhoneNumber" value='<%=PhoneNumber_ %>' size="25" maxlength="30" />&nbsp;(e.g: +3851XXX)</td>
	</tr>
	<tr>
	  <td>Phone Type</td>
	  <td width="1%">:</td>
...
<%
			Dim EmpRS
			strsql = "select EmpID, EmpName, Office from vwPhoneCustomerList Where Type='AMER' order by EmpName"
			'response.write strsql & "<br>"
...
%>
	...
	<tr>
	  <td>Customer No.</td>
	  <td width="1%">:</td>
	  <td><input type="input" name="txtCustNo" value='<%=CustNo_ %>' size="25" maxlength="20" /></td>
	</tr>
	<tr>
	  <td>Customer Name</td>
	  <td width="1%">:</td>
	  <td><input type="input" name="txtCustName" value='<%=CustName_ %>' size="50" maxlength="50" /></td>
	</tr>
	<tr>
	  <td valign="top">Address :</td>
	  <td width="1%">:</td>
	  <td>
		<textarea name="txtAddress" cols="60" rows="3"><%=Address_ %></textarea>
	</tr>
	<tr>
	  <td>City</td>
	  <td width="1%">:</td>
	  <td><input type="input" name="txtCity" value='<%=City_ %>' size="50" maxlength="50" /></td>
	</tr>
	<tr>
	  <td>Email Address</td>
	  <td width="1%">:</td>
...
-->
	...
	<tr>
	  ...
	  <td width="1%">:</td>
	  <td>
		  <select name="NoticeFlagList">
			<option value="Yes" <%if NoticeFlag_ ="Y" Then %>Selected<%End If%>>Yes</option>
			<option value="No" <%if NoticeFlag_ ="N" Then %>Selected<%End If%>>No</option>
		  </select>
	  </td>
	</tr>
	<tr>

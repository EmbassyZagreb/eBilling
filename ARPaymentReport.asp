...
function ClearFilter()
{
	...
	document.forms['frmSearch'].elements['cmbPaymentType'].value="X";
}
...
if PaymentType_ = "" Then PaymentType_ = Request.Form("cmbPaymentType")
if PaymentType_ = "" then
	PaymentType_ = "X"
end if
%>
...
<!--#include file="Header.inc" -->
  <TR>
  	<TD COLSPAN="4" ALIGN="center" Class="title">A/R PAYMENT REPORT</TD>
   </TR>
<tr>
...
%>
<form method="post" name="frmSearch" Action="ARPaymentReport.asp">
...
-->
			...
			<td>
				<Select name="cmbPaymentType">
					<option value="X" <%if PaymentType_ ="X" then %>Selected<%End If%>>-All--</option>
					<option value="F" <%if PaymentType_ ="F" then %>Selected<%End If%>>Full Payment</option>
					<option value="P" <%if PaymentType_ ="P" then %>Selected<%End If%>>Partial Payment</option>
				</select>
			</td>	
		</tr>
<!--
		<tr>
			<td align="right">&nbsp;Receipt Type&nbsp;</td>
			<td>:</td>
			<td>
				<Select name="cmbReceiptType">
					<Option value="X" <%if ReceiptType_ ="X" then %>Selected<%End If%>>-All-</Option>
					<Option value="5" <%if ReceiptType_ = "5" then %>Selected<%End If%>>Paid Partial</Option>
					<Option value="6" <%if ReceiptType_ = "6" then %>Selected<%End If%>>Full Partial / Completed</Option>
				</Select>&nbsp;
...
<div align="right"><input type="button" name="btnExport" value="Export to Excel" onClick="javascript:document.location.href('ARPaymentReportPrint.asp?sMonthP=<%=sMonthP%>&sYearP=<%=sYearP%>&eMonthP=<%=eMonthP%>&eYearP=<%=eYearP%>&Agency=<%=Agency_%>&Section=<%=Section_%>&EmpID=<%=EmpID_%>&PaymentType=<%=PaymentType_%>');"/></div>
<table border="1" bordercolor="#EEEEEE" cellpadding="2" cellspacing="0" width="100%"  class="FontText">
    <TR BGCOLOR="#330099" align="center">
         <TD width="2%" align="right"><strong><label STYLE=color:#FFFFFF>No.</label></strong></TD>
	 ...
	 <TD width="5%"><strong><label STYLE=color:#FFFFFF>Number</label></strong></TD>
         <TD width="6%"><strong><label STYLE=color:#FFFFFF>Billing<br>Period</label></strong></TD>
         <TD width="6%"><strong><label STYLE=color:#FFFFFF>Section</label></strong></TD>
	 <TD width="7%"><strong><label STYLE=color:#FFFFFF>Billing Amount (Kn.)</label></strong></TD>
         <TD width="8%"><strong><label STYLE=color:#FFFFFF>Paid Date</label></strong></TD>
         <TD width="5%"><strong><label STYLE=color:#FFFFFF>Receipt No.</label></strong></TD>
         <TD width="8%"><strong><label STYLE=color:#FFFFFF>Receipt Type</label></strong></TD>
         <TD width="5%"><strong><label STYLE=color:#FFFFFF>Receipt<br>Amount</label></strong></TD>
<!--         <TD><strong><label STYLE=color:#FFFFFF>Receipt<br>Currency</label></strong></TD> -->
         <TD width="5%"><strong><label STYLE=color:#FFFFFF>Age<br>(day)</label></strong></TD>
...
<!--     <tr BGCOLOR="#330099" align="center">
        <TD><strong><label STYLE=color:#FFFFFF>Home Phone</label></strong></TD>
         <TD width="5%"><strong><label STYLE=color:#FFFFFF>Office Phone</label></strong></TD>
         <TD width="5%"><strong><label STYLE=color:#FFFFFF>Mobile Phone</label></strong></TD>
         <TD><strong><label STYLE=color:#FFFFFF>Shuttle Bus</label></strong></TD>
         <TD width="5%"><strong><label STYLE=color:#FFFFFF>Total</label></strong></TD>
    </tr> -->
...
%>		
			...
			<%Else%>
				<a href="ARPaymentReport.asp?PageIndex=<%=PageNo%>&sMonthP=<%=sMonthP%>&sYearP=<%=sYearP%>&eMonthP=<%=eMonthP%>&eYearP=<%=eYearP%>&Agency=<%=Agency_%>&Section=<%=Section_%>&EmpID=<%=EmpID_%>&PaymentType=<%=PaymentType_%>"><%=PageNo%></a>&nbsp;
<%	
			End If						
			PageNo=PageNo+1
		Loop

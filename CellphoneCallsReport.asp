...
%>
		...
		<td colspan="2" align="center">
			...
			<tr>
				...
				<td>:</td>
		        	<td>
				<%
					strsql = "Select EmpID, EmpName from vwPhoneCustomerList Where len(rtrim(EmpName))>0 Order by EmpName"
	...
	<form method="post" name="frmCellphoneCallsList" action="CellphoneCallsPrint.asp?StartDate=<%=StartDate_ %>&EndDate=<%=EndDate_%>&Post=<%=Post_%>&Agency=<%=Agency_%>&Office=<%=Office_%>&EmpId=<%=EmpId_%>&PhoneType=<%=PhoneType_%>&CallType=<%=CallType_%>&SortBy=<%=SortBy_%>&Order=<%=Order_%>">
	<%
	...
	%>		
				...
				<%Else%>
					<a href="CellphoneCallsReport.asp?PageIndex=<%=PageNo%>&StartDate=<%=StartDate_ %>&EndDate=<%=EndDate_%>&Post=<%=Post_%>&Agency=<%=Agency_%>&Office=<%=Office_%>&EmpId=<%=EmpId_%>&PhoneType=<%=PhoneType_%>&CallType=<%=CallType_%>&SortBy=<%=SortBy_%>&Order=<%=Order_%>"><%=PageNo%></a>&nbsp;
	<%	
				End If						
				PageNo=PageNo+1
			Loop
